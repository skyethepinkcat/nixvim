        ...
        let
          ...
          mkArchive =
            ...
            pkgs.runCommand "nixvim-config-export-archive"
              {
                nativeBuildInputs = [ derv ];
              }
              ''
                mkdir -p $out
                mkdir -p tmp
                tar -chzf $out/nvim.tar.gz --no-same-permissions -C ${derv} .

              '';
          mkExportConfiguration =
            in_config:
          ...
          # Actual packages
          baseExports = lib.concatMapAttrs (name: value: {
            "${name}-export" = mkExportConfiguration value;
          }) config.nixvimConfigurations;
          archiveExports = lib.concatMapAttrs (name: value: {

            "${name}-archive" = mkArchive value;
          }) baseExports;

        in
        baseExports // archiveExports;
    };
}
